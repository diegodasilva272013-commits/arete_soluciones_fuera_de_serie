...
export default function MusicHero({
  ...
  // Ancla los listeners de rueda/touch al propio componente en vez de
  // "window" (como estaba en el original) — si escuchan en toda la
  // ventana, cualquier scroll en cualquier parte de una pagina mas
  // larga queda secuestrado por la lista de tracks para siempre, y el
  // resto de la pagina (todo lo que va despues de este hero) se vuelve
  // inalcanzable. Con el listener en el propio elemento, el gesto de
  // scroll/tilt/lista sigue funcionando igual, pero solo mientras el
  // cursor/dedo esta arriba del componente — apenas se sale, el scroll
  // normal de la pagina sigue andando. Es el unico cambio de
  // comportamiento sobre el archivo pegado; todo lo demas (video,
  // fisica, tilt, lista, sonido) queda igual.
  ...
  useEffect(() => {
    const onWheel = (e: WheelEvent) => {
      // Sin preventDefault a proposito: el original bloqueaba TODO
      // scroll de la pagina mientras el cursor estuviera sobre el
      // hero, y como el hero mide 100dvh y arranca en el tope de la
      // pagina, "sobre el hero" es literalmente toda la pantalla en
      // ese momento — no quedaba ni un pixel libre para scrollear mas
      // alla, la pagina quedaba trabada ahi para siempre. Dejando que
      // el scroll nativo siga su curso, la misma rueda mueve la lista
      // de tracks (efecto intacto) Y la pagina avanza normalmente.
      snapTargetRef.current = null
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      // Mismo motivo que en onWheel: sin preventDefault, para que el
      // dedo pueda seguir scrolleando la pagina de verdad ademas de
      // mover la lista.
  ...
  if (isCoarsePointer) {
    return (
      <div
        ...
        style={{
          // "fixed" en el original tapaba la pantalla entera para
          // siempre, sin importar el scroll — "relative" lo deja como
          // una seccion mas de una pagina mas larga, mismo tamaño
          // (100dvh), pero se va scrolleando como corresponde.
          position: "relative",
          height: "100dvh",
          width: "100%",
          background: bgVar,
    ...
    >
      ...
      <div
        style={{
          ...
          // En fullscreen, este wrapper pasa a ser el ancla real para
          // el titulo/firma/tarjeta (todos position:absolute) — sin
          // esto, al ser un item de flex sin stretch, quedaria del
          // tamaño de su contenido en vez de ocupar toda la seccion,
          // y "top/left:0" de sus hijos absolutos apuntaria a un
          // rectangulo casi sin alto.
          width: effectiveFullscreen ? "100%" : undefined,
      ...
      >
        ...
        <div style={{ position: "relative", width: effectiveFullscreen ? "100%" : undefined, height: effectiveFullscreen ? "100%" : undefined }}>
          ...
          <div
            ...
            style={{
              // "fixed" (viewport) -> "absolute" (dentro de la propia
              // seccion): mismo tamaño de pantalla completa (100vw/
              // 100dvh, unidades de viewport, no dependen del tamaño
              // del contenedor), pero ahora se va con el scroll de la
              // pagina en vez de quedar clavado. "top/left" en vez de
              // "inset" a proposito: inset fuerza el tamaño por los 4
              // bordes e ignora el width/height explicito si el
              // contenedor colapsó a 0 (los wrappers de arriba no
              // tienen tamaño propio, solo lo hereda del hijo).
              position: effectiveFullscreen ? "absolute" : "relative",
              top: effectiveFullscreen ? 0 : undefined,
              left: effectiveFullscreen ? 0 : undefined,
...
// ── PlayerControls ──────────────────────────────────────────
// Nunca llegó en el paste original (el archivo se corta siempre en el
// mismo punto, dos veces, por el límite de tamaño de mensaje) — esta es
// la única pieza reconstruida, a partir de como se la invoca en el
// resto del archivo (mismas props, mismos tokens de color/tipografia
// que el resto: SANS, fgMutedVar, CYAN, AMBER, la pastilla
// rgba(10,14,26,.55) con backdrop-blur que ya usan los otros botones).
function PlayerControls({
  ...
  const playSize = compact ? 48 : 54

  const pill: React.CSSProperties = {
    background: "rgba(10,14,26,0.55)",
    backdropFilter: "blur(10px)",
    WebkitBackdropFilter: "blur(10px)",
    border: `1px solid ${fgMutedVar(0.25)}`,
    borderRadius: 999,
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    color: fgMutedVar(0.85),
    cursor: "pointer",
  }
  ...
  return (
    <div
      style={{
        ...
        alignItems: "center",
        gap: 10,
        padding: compact ? "0 16px 22px" : "0 24px 26px",
      }}
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <button
          ...
          style={{ ...pill, width: btnSize, height: btnSize }}
        >
        ...
        <button
          ...
          style={{
            ...
            width: playSize,
            height: playSize,
            background: track.colorA,
            border: "none",
            color: "#05060a",
            boxShadow: `0 4px 20px ${track.colorA}66`,
          }}
        >
          {isPlaying ? (
      ...
      {showVideoControls && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2 }}>
          <button
            ...
            aria-label={videoSoundOn ? "Mute video ambience" : "Unmute video ambience"}
            style={{ ...pill, width: 30, height: 30, background: "transparent", border: "none", color: videoSoundOn ? CYAN : fgMutedVar(0.45) }}
          >
            {videoSoundOn ? (
          ...
          <input
            ...
            step={0.01}
            value={videoVolume}
            onChange={(e) => onVideoVolumeChange(clamp(Number(e.target.value), 0, MAX_VIDEO_VOLUME))}
            aria-label="Video ambience volume"
            style={{ width: 70, accentColor: CYAN }}
          />
        </div>
      )}
    </div>
